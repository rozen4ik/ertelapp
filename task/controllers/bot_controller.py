...
class BotController:
    ...
    def get_location(self, message) -> str:
        ...
        result = dadata.geolocate(name="address", lat=message.location.latitude, lon=message.location.longitude,
                                  count=1)
        result = result[0]
        result = result["value"]
        return result
    ...
    def create_work_task(self, task_id, msg_local, message):
        ...
        work_task.address_work_task = self.get_location(message)
